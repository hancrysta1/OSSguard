...
| 타이포스쿼팅 | 비교 대상 10개, 알고리즘 1개 (절반만 탐지) | 비교 대상 100+개, 알고리즘 4개 (표본 12건 전건 탐지) |
...
| 분석 성능 | YARA 룰 파일별 반복 컴파일, LLM 없음 | YARA 1회 컴파일 + LLM 병렬화 (11분→3분30초, 70% 단축) |
...
- Recall 50% → 100%, F1 Score 66.7% → 100%로 개선되었다 (표본 28개 — 악성 12 / 정상 16).
